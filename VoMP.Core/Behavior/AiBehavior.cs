...
namespace VoMP.Core.Behavior
{
    public class AiBehavior : IBehavior
    {
        ...
        public IActionChoice ChooseAction(Player player, List<IAction> validActions)
        {
            State = new AiState(player) {ValidActions = validActions};
            ...
            var completeContract = CompleteContractBehavior.CompleteContract(State, c => c.Reward.Move == 0, null);
            ...
            // Low hanging fruit
            var khansFavor = GenerateResourcesBehavior.UseKhansFavor(State, ResourceType.Gold);
