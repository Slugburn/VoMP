...
namespace VoMP.Core.Behavior
{
    class TakeContractsBehavior
    {
        public static IActionChoice TakeContracts(AiState state, Func<Contract, bool> condition)
        {
            ...
            takeContracts.Die = availableDice.GetLowest(d => d.Value >= takeContracts.Contracts.Max(x => x.Value));
        ...
        private static ContractSpace GetEasiestContract(Player player, ResourceBag resources, SpaceAction space, IEnumerable<ContractSpace> contracts, IEnumerable<Die> availableDice)
        {
